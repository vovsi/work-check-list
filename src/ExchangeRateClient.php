...
/**
 * Курс USD → валюта из [currency] для EarningsService — адрес API берётся из конфига
 * ([services].exchange_rate_url, по умолчанию open.er-api.com: бесплатный публичный API без ключа).
...
final class ExchangeRateClient
{
    ...
    public function rateFromUsd(): float
    {
    ...
    private function fetchRate(): float
    {
        ...
        $rate = $data['rates'][$this->currencyCode] ?? null;
        if (!is_numeric($rate)) {
            throw new RuntimeException("Сервис курсов валют не вернул курс {$this->currencyCode}");
        }
    ...
    private function readCache(): ?array
    {
        ...
        // Курс закэширован в другой валюте (сменили [currency] в конфиге) — кэш непригоден
        if (($data['currency'] ?? '') !== $this->currencyCode) {
            return null;
        }
