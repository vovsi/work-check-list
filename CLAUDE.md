...
- **`README.md` держим актуальным — это часть задачи, а не отдельная просьба.** README —
  ...
  коммите (разделы «Конфигурация», «Шаги чек-листа», «Проверка, что всё поднялось»,
  «Траблшутинг»). Описание архитектуры и внутренних правил остаётся здесь, в `CLAUDE.md`;
  в README идёт только то, что нужно пользователю для запуска и настройки. Скриншоты —
