...
- **Service (оркестрация use-case)** — `TaskService::findOrCreateByLink()`. Инкапсулирует
  ...
  дольше `STALE_PULL_REQUEST_HOURS` в статусе Pull request» живёт в нём, а не в `api/dashboard.php`,
  и опциональность Jira инкапсулирована в том же фабричном методе `createFromConfig()`.
...
- **Режим Claude Code Skill** (`[mode].claude_code_skill_mode` = `1`) — пункты из
  ...
  **не удалены** из
  ...
  так у `send_pr` пропадает кнопка «Скопировать Link to PR», у `jira_description` — кнопка
  «Скопировать PR», а в шаблоне выливки строка «3. Вылить» остаётся без ссылки и без подсказки
  вставить её вручную (`PR_LINK_MISSING_HINT` подставляется только когда пункт `pull_request`
  виден). Добавляя новую зависимость между пунктами — проверяй наличие пункта-источника этой
  же функцией, а не считай его существующим всегда.
...
| `send_pr` | Отправить PR ревьюверу | Telegram | Модалка без текста-превью: кнопки копирования «Скопировать Link to PR» (только если пункт `pull_request` не скрыт режимом Claude Code Skill) / «Скопировать Details template» (шаблон с `task_id`, ссылкой на PR из шага `pull_request` и названием проекта из `[templates].deploy_config_project` в строке «Добавить в конфиг …») в теле окна — не отмечают пункт, можно повторять → «Готово» в панели действий отмечает |
...
(`atlassian.pull_request_status`) дольше `DashboardService::STALE_PULL_REQUEST_HOURS` (24 ч):
`JiraClient::fetchIssuesStuckInStatus()` ищет их JQL-запросом
`assignee = currentUser() AND status = "…" AND status CHANGED TO "…" BEFORE "-24h"` (через тот же
`/rest/api/2/search/jql`, что и сегодняшние ворклоги; выборка ограничена 100 задачами).
...
категория, дата не обновляется). Задачу, успевшую выйти из статуса и вернуться в него за
последние 24 ч, выборка тоже покажет — точность здесь не стоит запроса changelog по каждой
задаче. Порог в часах — константа класса, а не ключ конфига: это не данные команды, а условие
самого показателя. 422 — интеграция с Jira не настроена; 502 — ошибка Jira.
...
полученных данных, второго запроса к Jira нет. Название статуса и порог тоже приходят с сервера:
подпись и подсказка показателя собираются из них, на фронте не зашиты.

Вызывается из `loadDashboard()` в `app.js` — при инициализации (независимо от загрузки задачи,
как `loadTodayTimeSpent()`) и по кнопке актуализации в блоке дашборда.
...
- Дашборд показателей (`.dashboard`, экран ввода ссылки, между заголовком и полем ссылки) —
  ...
  в статусе Pull request дольше 24 ч. Плитка показателя кликабельна — открывает модалку со
  ...
  приём, что у индикатора времени). Индикация: спиннер в слоте значения только на первой
  загрузке, при актуализации — `setButtonLoading` на самой кнопке обновления. Ошибка запроса
...
- Возврат к экрану ввода ссылки — стеклянный кружок со стрелкой (`.back-btn`,
  ...
  `showLinkScreen()`, т.е. кнопка существует только внутри задачи.
